...
// 为每个 Agent（Codex/Claude/Claude Desktop/OpenClaw/Gemini）注册 uTools 动态命令，
// 在搜索框输入 `cc {Agent名}` 直接打开插件并切到对应 Agent 页签

import { ref } from 'vue'
import { APP_LABELS, APP_TYPES } from './shared'
import { useProviders } from './useProviders'
import type { AppType } from '../types/utools-cctoggle'
...
function isUtoolsEnv(): boolean {
  return typeof utools !== 'undefined' && typeof utools.setFeature === 'function'
}
...
function persistConfig(cfg: QuickSwitchConfig): void {
  try {
    utools?.dbStorage?.setItem(STORAGE_KEY, cfg)
  } catch (e) { /* ignore */ }
}
...
function registerFor(appType: AppType): void {
  if (!isUtoolsEnv() || !isEnabled()) return
  setFeature(appType)
}
...
function executeSwitch(code: string): boolean {
  ...
  try {
    ...
    utools.showNotification(`已切换到 ${APP_LABELS[appType] || appType}`)
  } catch (e) {
