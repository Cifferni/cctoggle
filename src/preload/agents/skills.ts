...
export class SkillManager {
  ...
  static getNestDir(): string {
    // 优先从配置读取（用户自定义路径）
    var configured: string | null = utools.dbStorage.getItem('ccswitch_nest_dir');
    if (configured) {
  ...
  static getSkillStoragePaths(): Record<string, string> {
    ...
    // 兼容旧的独立存储路径配置（向后兼容）
    var saved: Record<string, string> | null = utools.dbStorage.getItem('ccswitch_skill_paths');
    if (saved) {
      // 如果旧数据存在，尝试迁移
  ...
  static removeSkillRepo(repoUrl: string): OperationResult {
    utools.dbStorage.setItem('ccswitch_skill_repos', SkillManager.getSkillRepos().filter(function(r) { return r.url !== repoUrl; }));
    return { success: true };
  }
