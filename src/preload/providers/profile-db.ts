...
export class ProfileStore {
  ...
  static listProfiles(): ProjectProfile[] {
    try {
      const docs = utools.db.allDocs(PROFILE_PREFIX) || [];
      return docs
  ...
  static getProfile(id: string): ProjectProfile | null {
    try {
      const doc = utools.db.get(ProfileStore.getProfileKey(id));
      if (!doc) return null;
      return {
        id: id,
        name: doc.name || "",
  ...
  static deleteProfile(id: string): void {
    ...
    utools.db.remove(ProfileStore.getProfileKey(id));
  }
  ...
  static activateProfile(id: string): { success: boolean; error?: string } {
    ...
    try {
      ...
      // 持久化激活状态
      utools.dbStorage.setItem(ACTIVE_PROFILE_KEY, id);
  ...
  static deactivateProfile(): void {
    try {
      utools.dbStorage.removeItem(ACTIVE_PROFILE_KEY);
    } catch (e) {}
  }
  ...
  static getActiveProfileId(): string | null {
    try {
      const raw = utools.dbStorage.getItem(ACTIVE_PROFILE_KEY);
