...
export class ProxyManager {
  ...
  static _backupCurrent(appType: string): void {
    ...
    const doc: any = utools.db.get(BACKUP_KEY) || { _id: BACKUP_KEY };
  ...
  static getRouteGroup(appType: string, id: string): RouteGroup | null {
    try {
      const doc = utools.db.get(ProxyManager._routeKey(appType, id));
      if (!doc) return null;
  ...
  static deleteRouteGroup(appType: string, id: string): boolean {
    try {
      ...
      utools.db.remove(ProxyManager._routeKey(appType, id));
      return true;
