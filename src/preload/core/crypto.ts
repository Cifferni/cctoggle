...
function persistMasterKey(key: Buffer): void {
  ...
  try {
    if (safe) {
      ...
      utools.dbStorage.setItem(MASTER_KEY_STORAGE, "enc:" + blob);
      return;
    }
  } catch (e) {
  ...
  try {
    utools.dbCryptoStorage.setItem(MASTER_KEY_STORAGE, key.toString("base64"));
  } catch (e) {
...
function loadMasterKey(): Buffer | null {
  ...
  try {
    const raw = utools.dbStorage.getItem(MASTER_KEY_STORAGE);
    if (raw) {
  ...
  try {
    const raw = utools.dbCryptoStorage.getItem(MASTER_KEY_STORAGE);
    if (raw) {
...
export function isCryptoStorageKey(appType: string, providerId: string): boolean {
  try {
    const raw = utools.dbCryptoStorage.getItem("apikey_" + appType + "_" + providerId);
    return !!raw;
  } catch (e) {
    return false;
  }
...
export function getCryptoStorageKey(appType: string, providerId: string): string {
  try {
    return utools.dbCryptoStorage.getItem("apikey_" + appType + "_" + providerId) || "";
  } catch (e) {
    return "";
  }
}
