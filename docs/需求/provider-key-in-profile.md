...
| 数据 | 存储位置 | 说明 |
|------|---------|------|
| 供应商配置 | profile（`utools.db`，明文） | `cctoggle_profile_{profileId}` |
| API Key | `utools.dbCryptoStorage`（加密） | key 名 `apikey_{appType}_{providerId}` |
...
- **不再调用** `utools.dbCryptoStorage.setItem(...)`。
...
- **不再调用** `utools.dbCryptoStorage.getItem(...)`。
...
- **不再调用** `utools.dbCryptoStorage.removeItem(...)`。
