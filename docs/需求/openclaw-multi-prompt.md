...
> ⚠️ 按 API 同步规则：新增/修改 preload 方法时，需同步更新 `preload.ts`、`utools-cctoggle.d.ts`、`browser-adapter.ts`，真实数据类还需更新 `dev-api-server.cjs`。
...
| 3 | API 同步（3 文件规则 + dev-api-server） | `preload.ts`、`utools-cctoggle.d.ts`、`browser-adapter.ts`、`dev-api-server.cjs` |
...
- uTools 环境手动验证：新建 OpenClaw 提示词选 SOUL.md → 应用到卡片 chip → 检查 `workspace/SOUL.md` 内容
- 验证 MEMORY.md 全程不被触碰
- 旧格式备份迁移后恢复正确
