...
在供应商卡片上显示该供应商的**账户余额**。余额查询接口由**用户在每个供应商上手动配置**（请求路径 + 取值规则），内置常见厂商模板供一键套用。余额不足时卡片变色告警，并通过 uTools 系统通知提醒用户充值。
...
| 低余额阈值 | 余额低于该值时卡片标红 + uTools 系统通知（默认 5，按供应商配置） |
...
| Key | 存储 | 内容 |
|-----|------|------|
| Provider 的 `balance` 字段 | utools.db | ProviderBalanceConfig（随供应商导入导出） |
| `cctoggle_balance_cache` | utools.dbStorage | `Record<providerId, BalanceCacheEntry>` |
...
- uTools 通知：**仅当前激活供应商**在以下时机触发一次 `utools.showNotification`：
  - 启动自动查询后首次发现余额低于阈值
  - 手动刷新后余额从高于阈值变为低于阈值
...
> 按 API 同步规则，需同步更新 4 处：`preload.ts`、`utools-cctoggle.d.ts`、`browser-adapter.ts`、`dev-api-server.cjs`。
...
4. 余额低于阈值时区块文字变红 + ⚠ 警示，当前激活供应商弹出 uTools 通知且同会话只通知一次
