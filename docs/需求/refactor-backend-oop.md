...
| 文件 | 行数 | 职责 |
|---|---|---|
| `services.ts` | 178 | 入口文件，组装所有模块到 `window.utoolsCctoggle` |
...
2. 定义核心接口/类型（扩展现有 `src/types/utools-cctoggle.d.ts`）
3. 建立基类 `BaseStore`（封装 `utools.db` 操作的通用 CRUD）
...
1. **uTools preload 环境限制**：preload 运行在 Electron renderer 的 preload 上下文，不是完整 Node.js 环境。确保 class 语法在目标 Electron 版本中受支持（ES2020 target 已覆盖）
2. **`proxy-daemon.ts` 运行在隐藏 BrowserWindow**：它有独立的 preload 入口，改造时需确保 IPC 通信接口（`utools.sendToParent`）不变
3. **向后兼容**：`services.ts` 暴露到 `window.utoolsCctoggle` 的 API 必须保持不变，前端 Vue 代码不应受影响
...
- 每个模块改造后，运行现有功能手动测试（uTools 插件环境）
- 确保 `window.utoolsCctoggle` 的所有方法签名不变
