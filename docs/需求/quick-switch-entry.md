...
在 uTools 搜索框内直接切换到某个 Agent（Codex / Claude / Claude Desktop / OpenClaw / Gemini）。输入 `cc {Agent名}` 匹配到对应动态命令，回车后打开插件主界面并停留在该 Agent 页签，无需手动点顶栏 tab。
...
uTools 提供「动态命令（setFeature）」能力：为每个 Agent 注册一条搜索命令，输入 `cc {名称}` 即出结果，选中即打开对应 Agent 页签，路径最短、原生搜索体验。
...
| 概念 | 说明 |
|------|------|
| 快捷命令 | 为每个 Agent 注册一条 uTools 动态命令，选择即打开对应 Agent 页签 |
...
code = ccs_switch_{appType}
  ...
  → utools.showNotification(`已切换到 ${Agent显示名}`)
...
> 统一收敛到一个 `useQuickSwitch` composable：内部维护 `reconcile()`（先清已注册的 `ccs_switch_*` 命令，再按全部 Agent 全量注册），页面无需关心细节。**副作用仅作用于 uTools 环境**，浏览器模式自动跳过。

### 2. 匹配与结果反馈

- 匹配维度：Agent 显示名（如 `Codex` / `Claude Desktop`），uTools 原生子串匹配、忽略大小写
...
> 配置存 `utools.dbStorage`（key: `cctoggle_quick_switch`），浏览器模式回退默认值。
...
- 命令注册/清理用 uTools 原生 `utools.setFeature` / `removeFeature`（前端可直接调用，不走 preload）。

> 不修改 `preload.ts` / `utools-cctoggle.d.ts` / `browser-adapter.ts` / `dev-api-server.cjs`（本期无 preload API 变更）。
...
| 场景 | 处理 |
|------|------|
| Agent 显示名含空格/特殊字符 | 命令关键词原样保留，uTools 按子串匹配，不影响 |
| 浏览器开发模式 | `utools` 不存在，`useQuickSwitch` 直接跳过，不影响页面 |
...
1. 不做全局热键（uTools 本身无热键注册能力）
2. 不做搜索历史 / 最近使用排序
3. 不把动态命令逻辑下沉到 preload（前端直接调 uTools API 足够）
4. 不做命令关键词模糊匹配算法（依赖 uTools 原生子串匹配）
