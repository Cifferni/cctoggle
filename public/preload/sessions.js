    ...
    if (Array.isArray(content)) {
        ...
        for (var i = 0; i < content.length; i++) {
            ...
            if (item.type === "text" && item.text)
                ...
                parts.push("[思考: " + (item.thinking || "").substring(0, 100) + "...]");
        }
        return parts.join("");
    }
...
function _parseOpenClawMessages(lines) {
    ...
    for (var i = 0; i < lines.length; i++) {
        ...
        if (content)
            messages.push({ role: role, content: content, timestamp: d.timestamp || "" });
    }
    return messages;
}
// 解析 Codex 消息
...
function _parseClaudeMessages(lines) {
    ...
    for (var i = 0; i < lines.length; i++) {
        ...
        if (d.type === "human" || d.type === "user") {
            ...
            if (c)
                messages.push({ role: "user", content: c, timestamp: d.timestamp || "" });
        }
        else if (d.type === "assistant") {
            var ac = d.message ? _extractContent(d.message.content) : _extractContent(d.content);
            if (ac)
                messages.push({ role: "assistant", content: ac, timestamp: d.timestamp || "" });
        }
    }
    return messages;
}
